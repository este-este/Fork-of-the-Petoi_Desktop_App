...
# Class Refactoring and Other Modifications
    ...
    # v001.00.00
'''
    The original file, translate.py, is a simple list of module attributes.  The refactored version, translate_C.py, has a self-contained class, Translate, that encapsulates that information using class attributes and includes the function txt() to return the translated text.
'''
...
class Translate:
    def __init__(self, callerName):     # Note the parameters that are passed to the constructor.
        ...
        Purpose / Responsibility:
            Create an object to hold translation information.
        ...
        self.versionNum = '1.2.4'       # This is the original Petoi Desktop App version number
        self.dateStr = '2025-04-08'     # This is the original Petoi Desktop App release date
    ...
    'Debugger': 'Tools',
    'tipDebugger':'Provide convenient tools to debug the robot.',
...
The program will automatically identify the correct serial port name.''',
    ...
    # TD-ee   Added this as a label in ArdSerial.manualSelect(), so also need this in all the language dictionaries
    '(Click "OK" with no port selected to trigger a scan of ALL ports.  \n\
Selecting ports with no Petoi devices will also trigger a scan of ALL ports.)':
    '(Click "OK" with no port selected to trigger a scan of ALL ports.  \n\
Selecting ports with no Petoi devices will also trigger a scan of ALL ports.)',
 ...
 * You can select \'N\' to preserve the calibration values. ''',
 ...
 The function of this button is the same as the function of the "Upgrade the Firmware" button.''',
    ...
    'msgVersion': 'Version: ' + self.versionNum + '\n'+
...
Then re-upgrade the firmware.\n''',
    ...
    'tipPlay':'Go through the scheduler from the active frame\nYou may stop while playing',
    ...
    'tipGorB':'A behavior interpolates between the key frames and executes only once\nA gait loops over all the frames without any smoothing',
 ...
 * คุณสามารถเลือก "N" เพื่อรักษาค่าการสอบเทียบ ''',
 ...
 ''',
    ...
    'msgVersion': 'เวอร์ชัน: ' + self.versionNum + '\n'+
...
問題を解決できない場合は、ログを support@petoi.com に送信してサポートを受けてください。''',
 ...
 このボタンの機能は「ファームウェアのアップグレード」ボタンの機能と同じです。''',
    ...
    'msgVersion':'バージョン：' + self.versionNum + '\n'+
 ...
 * È possibile selezionare \'N\' per conservare i valori di calibrazione. ''',
 ...
 La funzione di questo pulsante è la stessa del pulsante "Aggiorna il firmware".''',
    ...
    'msgVersion': 'Versione: ' + self.versionNum + '\n' +
 ...
 * Можно выбрать \'N\' для сохранения значений калибровки. ''',
 ...
 Функция этой кнопки аналогична функции кнопки «Обновить прошивку».''',
    ...
    'msgVersion': 'Версия: ' + self.versionNum + '\n'+
...
Затем повторно обновите прошивку.\n''',
    ...
    'poweron':'Робот должен питаться от батареи для вращения суставов, \n\nЭто ЖИЗНЕННО необходимо для контроля движения суставов!\n\nЖелтый светодиод на материнской плате должен загореться, \n\иначе подключите аккумулятор и нажмите и удерживайте эту \n\кнопку 3 секунды для вкючения робота.\n\n',
        ...
        # languageList is a dictionary of dictionaries:
            # The outer dictionary uses a language string as a key and a dictionary of text in that language as a value.  E.g. 'French':self.textFR
                # The inner dictionary uses an English descriptor string as a key and a specified language representation as a value (e.g. 'Creator':'Créateur')
        self.languageList = {
    'English':self.textEN,
    'Simplified Chinese':self.textCN,
    'Traditional Chinese':self.textCN_TW,
    'German':self.textDE,
    'Thai':self.textTH,
    'French':self.textFR,
    'Japanese':self.textJP,
    'Italian':self.textIT,
    'Russian':self.textRU,
    #to be added
}

        self.strLan = 'English'                             # default value (Str)
        self.language = self.languageList[self.strLan]      # default value (Dictionary Str:Str)

    def txt(self, key):
        return self.language.get(key, self.textEN[key])
